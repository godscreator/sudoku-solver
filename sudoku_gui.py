...
class board(object):
    def __init__(self, asset_folder):
        ...
        self.initial_state = [["-"] * 9 for _ in range(9)]
        self.current_state = [["-"] * 9 for _ in range(9)]
        self.color_state = [[1] * 9 for _ in range(9)]
        self.S = "123456789-"

        # set attribute to value specific to image files.
        self.sudoku_board_image = None
        self.thick_line_width = 0
        self.thin_line_width = 0
        self.small_grid_side = 0
    ...
    def load_num_images(self, asset_folder, color):
        ...
        return list(
            map(pygame.image.load, list(map(set_path, list(map(lambda i: i + "_" + color + ".png", list(self.S)))))))
    ...
    def xy_to_grid(self, x, y):
        return ((y - self.thin_line_width - self.thick_line_width) // (
                    self.small_grid_side + self.thin_line_width + self.thick_line_width // 3),
                (x - self.thin_line_width - self.thick_line_width) // (
                            self.small_grid_side + self.thin_line_width + self.thick_line_width // 3))
    ...
    def set_current_from(self, state):
        self.current_state = [[state[i][j] for j in range(9)] for i in range(9)]
        self.color_state = [[1] * 9 for _ in range(9)]
...
class button:
    def __init__(self, x, y, width, height, action, active=True, color=(0, 0, 0), text="",
                 font=pygame.font.get_default_font(), font_size=10, font_color=(255, 255, 255), img=None):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
